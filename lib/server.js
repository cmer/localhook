...
async function forwardWebhook(webhook, forwardTo) {
  ...
  // Identify forwarded requests and prevent loops
  headers['x-forwarded-by'] = `LocalHook/${VERSION}`;
...
function createServer(port, options = {}) {
  ...
  app.use((req, res, next) => {
    ...
    req.on('end', async () => {
      ...
      if (options.forwardTo && !req.headers['x-forwarded-by']?.startsWith('LocalHook')) {
  ...
  const server = app.listen(port, async () => {
    ...
    if (options.tailscale || options.cloudflare) {
      ...
      try {
        ...
        console.log(`
  ${bold}LocalHook ${VERSION}${reset} is running!
